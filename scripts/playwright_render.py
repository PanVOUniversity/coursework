...
async def render_page(width: int, height: int, html_path: Path, output_dir: Path, meta_dir: Path = None):
    ...
    async with async_playwright() as p:
        ...
        if meta_dir is not None:
            ...
            print(f"Generated metadata: {meta_path} ({len(metadata['frames'])} frames)")
...
def render(
    ...
    Args:
        ...
        workers: Number of parallel workers (not implemented for >1 in this stub).
    ...
    async def render_all():
        async def render_one(html_path):
            ...
            print(f"Rendered {html_path} -> {output_path} (resolution: {width}x{height})")

        # Sequential rendering (workers>1 not implemented here)
        for html_path in html_files:
            await render_one(html_path)
