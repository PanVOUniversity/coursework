...
"""
    ...
    metadata = {
        ...
        'actual_page_width': VIEWPORT_WIDTH,
...
def main():
    ...
    for page_id in range(1, args.n + 1):
        # Random number of sections (frames) for this page
        num_sections = random.randint(args.min_frames, args.max_frames)
        
        # Generate page (using default min/max blocks per section: 3-10)
        html, metadata = generate_full_page(page_id, num_sections, min_blocks_per_section=3, max_blocks_per_section=10)
    ...
    print(f"Done! Generated {args.n} pages")
    print(f"HTML files saved in {pages_dir}")
    print(f"Metadata files saved in {meta_dir}")
